...
namespace Student_Management_System
{
    ...
    public partial class Login : Form
    {
        ...
        private void userLogin()
        {
            ...
            {
                conn.Open();
                string sql = "SELECT passport, student_number, faculty, department, program, program_duration, first_name, last_name, email, phone, dob, gender, username, password, address FROM students " + 
                ...
                using (MySqlCommand cmd = new MySqlCommand(sql, conn))
                    {
                        ...
                        if (!string.IsNullOrEmpty(UserName.Text) && (!string.IsNullOrEmpty(password.Text)))
                        {
                            ...
                            if (reader.Read())
                            {
                                ...
                                {
                                    byte[] imgData = (byte[])reader["passport"];
                                ...
                                int programDuration = int.Parse(reader["program_duration"].ToString());
                                string programType = reader["program"].ToString();
                            string stdNumber = stdnu; // Example
                            int admissionYear = int.Parse(stdnu.Substring(0, 4));
                            int currentYear = DateTime.Now.Year;
                            int yearsPassed = currentYear - admissionYear;

                            // Check based on program duration
                            if (yearsPassed > programDuration)
                            {
                                MessageBox.Show($"You cannot sign in: your {programType} program duration has expired.");
                                return;
                            }
                            else
                            {
                                ...
                                UserName.Clear();
                                password.Clear();
                                Dashboard dash = new Dashboard(passportImage, stdnu, fac, dept, name, last, mail, phon, date, gen, user, pass, addr, programType, programDuration);
